...
export class ViewAttendancePrincipalComponent {
  ...
  constructor(private viewAttendancePrincipalService: ViewAttendancePrincipalComponent,  public dialog: MatDialog,  private attendanceService: AttendanceService ) { }
  ...
  findbyIdActivity(idActivity: any) {
    this.viewAttendancePrincipalService.findbyIdActivity(idActivity).subscribe( (res) => {
      this.dataSource = res;
  ...
  listActivities(id:any) {
    this.viewAttendanceService.listActivities(id).subscribe((res: any) => {
      this.activitiesList = res;
    })
  }

  listPrograms(){
    this.viewAttendanceService.listPrograms().subscribe((res:any) => {
